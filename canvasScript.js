...
var wordList = [
  // Borrowed from randomwords.js which Borrowed from xkcd password generator which borrowed it from wherever
...
var magePic = document.getElementById("mage");
var warrPic = document.getElementById("warrior");
context.drawImage(mage, 190, 120);
context.drawImage(warrior, 725, 100);
...
function beginGame() {
	document.addEventListener("keydown", function(y) {
		...
		if(userInput === 13 && !playing) {
			clearBox();
			moveEnemies();
			playing = true;
		}
		lastkey = String.fromCharCode(userInput).toLowerCase()
	});
}

function drawCharacter() {
	var smiley = document.getElementById("character");
	context.drawImage(smiley, 10, 70);
}

var fruitDefeated = false;
var currentEnemy = wordList[Math.floor(Math.random() * 1949)];
var score = 0;
var deadEnemyCount = 0;

function moveEnemies() {

	if(lastkey === currentEnemy[0] && playing) {
		currentEnemy = currentEnemy.slice(1);
		score += 50;
	}
    ...
    if (x <= 125) { 			//Make sure X doesn't go off the side
        x = 125;
    	playerLoss();
    }
    velX--;
    ...
    context.fillStyle = "white";
    drawCharacter();
    context.fillText(currentEnemy, (x - 5), 225);
    ...
    if(currentEnemy.length > 0) {
		...
		currentEnemy = wordList[Math.floor(Math.random() * 1949)];
		deadEnemyCount++;
    x = 700;
    velX = 0;
		enemiesDefeated();
...
function enemiesDefeated() {
		...
		if (deadEnemyCount > 0) {
			...
			context.font = "20px Arial";
			context.fillStyle="white";
			context.fillText("You win!", 80 , 450);	
			var stringScore = score.toString();
			context.fillText("Score: " + stringScore, 80 , 475);		
		};
...
function playerLoss() {
	...
	context.font = "20px Arial";
	context.fillStyle="white";
	context.fillText("you won the battle, but lost the war", 80 , 450);
}
